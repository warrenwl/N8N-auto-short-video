// n8n Code node: Validate Novel Review Manual Edit
// Only accepts POST body fields from /webhook/novel-review-manual-edit.

const source = $json || {};
if (source.query && !source.body) {
  throw new Error('审核改稿必须通过 POST body 提交，拒绝 GET/query 参数。');
}

if (!source.body || typeof source.body !== 'object') {
  throw new Error('审核改稿必须通过 POST body 提交。');
}
...
if (!reviewToken) {
  throw new Error('缺少 review_token，拒绝保存审核改稿。');
}
...
if (!decision) {
  throw new Error(`无效人工改稿决策：${rawDecision || '(empty)'}`);
}
...
return [{
  json: {
    ...
    reviewer,
    decision,
    action: 'MANUAL_EDIT_REVIEW_CHAPTER',
  },
}];
